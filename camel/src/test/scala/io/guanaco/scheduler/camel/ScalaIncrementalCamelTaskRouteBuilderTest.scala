...
import io.guanaco.scheduler.camel.CamelTaskItem.{Full, Incremental, Operation}
import io.guanaco.scheduler.Task
import io.guanaco.scheduler.camel.CamelTaskItem.Operation
...
/**
  ...
  */
class ScalaIncrementalCamelTaskRouteBuilderTest
    extends AbstractCamelSchedulerTest {
  ...
  def testTriggerSucceeds() {
    getMockEndpoint(MOCK_TRIGGERED).expectedBodiesReceived(
      MESSAGE_1,
      MESSAGE_2,
      MESSAGE_3
    )
  ...
  override protected def createRouteBuilder: RouteBuilder = {
    ...
    val builder: CamelTaskRouteBuilder = new CamelTaskRouteBuilder(
      "TestTask",
      "59 59 23 31 12 ? 2099",
      Some(500),
      alerta
